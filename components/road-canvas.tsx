...
export default function RoadCanvas({
  ...
  // Add missing drawing functions that are referenced but not defined
  ...
  const drawRoad = (ctx: CanvasRenderingContext2D, road: Road, isSelected: boolean, isSelectedForDisconnect: boolean) => {
    ctx.strokeStyle = isSelected ? "#3b82f6" : isSelectedForDisconnect ? "#ef4444" : "#374151"
    ...
    // Draw road name if enabled and road has a name
    if (showRoadNames && road.name && road.name.trim() !== "") {
      drawTextAlongPath(ctx, road.name, road)
    }

    // Draw road length if enabled
    if (showRoadLengths) {
      const length = calculateRoadLength(road)
      const lengthText = `${length.toFixed(1)}m`
      
      // Position length text below the road name or at road position if no name
      let offsetY = road.width / 2 + 15 / zoom
      if (showRoadNames && road.name && road.name.trim() !== "") {
        offsetY += 20 / zoom // Additional offset if name is shown
      }
      ...
      if (road.type === RoadType.STRAIGHT) {
        const midX = (road.start.x + road.end.x) / 2
        const midY = (road.start.y + road.end.y) / 2
        ctx.strokeText(lengthText, midX, midY + offsetY)
        ctx.fillText(lengthText, midX, midY + offsetY)
      } else if (road.type === RoadType.BEZIER && road.controlPoints) {
        const t = 0.5
        const mt = 1 - t
        const x = mt * mt * mt * road.start.x +
        ...
        const y = mt * mt * mt * road.start.y +
                  ...
                  3 * mt * t * t * road.controlPoints[1].y +
                  t * t * t * road.end.y
        ctx.strokeText(lengthText, x, y + offsetY)
        ctx.fillText(lengthText, x, y + offsetY)
      }
    }
  }

  const drawNode = (ctx: CanvasRenderingContext2D, node: Node, isSelected: boolean, isConnecting: boolean) => {
    ctx.fillStyle = isSelected ? "#3b82f6" : isConnecting ? "#10b981" : "#6b7280"
    ctx.strokeStyle = "#ffffff"
    ctx.lineWidth = 2 / zoom

    ctx.beginPath()
    ctx.arc(node.x, node.y, 6 / zoom, 0, Math.PI * 2)
    ctx.fill()
    ctx.stroke()

    // Draw larger highlight for selected or connecting nodes
    if (isSelected || isConnecting) {
      ctx.strokeStyle = isSelected ? "#3b82f6" : "#10b981"
      ctx.lineWidth = 2 / zoom
      ctx.beginPath()
      ctx.arc(node.x, node.y, 10 / zoom, 0, Math.PI * 2)
      ctx.stroke()
    }
  }

  const drawBuildSessionPreview = (ctx: CanvasRenderingContext2D, session: BuildSession, currentMousePos: { x: number; y: number } | null, isDrawingCurve: boolean) => {
    if (!session.isActive || !session.startNode || !currentMousePos) return

    ctx.strokeStyle = "#3b82f6"
    ctx.lineWidth = session.roadWidth || defaultRoadWidth
    ctx.lineCap = "round"
    ctx.setLineDash([5 / zoom, 5 / zoom])

    if (session.roadType === RoadType.STRAIGHT || !isDrawingCurve) {
      ctx.beginPath()
      ctx.moveTo(session.startNode.x, session.startNode.y)
      ctx.lineTo(currentMousePos.x, currentMousePos.y)
      ctx.stroke()
    } else if (session.roadType === RoadType.BEZIER && session.controlPoints) {
      ctx.beginPath()
      ctx.moveTo(session.startNode.x, session.startNode.y)
      ctx.bezierCurveTo(
        session.controlPoints[0].x,
        session.controlPoints[0].y,
        session.controlPoints[1].x,
        session.controlPoints[1].y,
        currentMousePos.x,
        currentMousePos.y
      )
      ctx.stroke()
      ...
      ctx.beginPath()
      ctx.moveTo(session.startNode.x, session.startNode.y)
      ctx.lineTo(session.controlPoints[0].x, session.controlPoints[0].y)
      ctx.stroke()
      
      // Line from end to second control point
      ctx.beginPath()
      ctx.moveTo(currentMousePos.x, currentMousePos.y)
      ctx.lineTo(session.controlPoints[1].x, session.controlPoints[1].y)
      ctx.stroke()

      // Draw control point circles
      ctx.fillStyle = "#fb923c"
      ctx.beginPath()
      ctx.arc(session.controlPoints[0].x, session.controlPoints[0].y, 4 / zoom, 0, Math.PI * 2)
      ctx.fill()
      ctx.beginPath()
      ctx.arc(session.controlPoints[1].x, session.controlPoints[1].y, 4 / zoom, 0, Math.PI * 2)
      ctx.fill()
    }
  ...
  return (
    <div ref={containerRef} className="relative flex-1 bg-white">
      <canvas ref={canvasRef} onMouseDown={onMouseDown} onMouseMove={onMouseMove} onMouseUp={onMouseUp} className={`w-full h-full ${getCursorClass()}`} />
      ...
      {selectedPolygon && onUpdatePolygonName && (
        ...
        >
          {editingPolygonName === selectedPolygon.id ? (
            ...
            <div
              className="group relative inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-purple-500 to-indigo-600 rounded-full shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200 cursor-pointer border-2 border-white/20 backdrop-blur-sm w-[200px]"
              onClick={() => handlePolygonNameClick(selectedPolygon.id, selectedPolygon.name || "")}
            >
              <div className="absolute inset-0 bg-gradient-to-r from-purple-400 to-indigo-500 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-200 blur-sm"></div>
              <span className="relative z-10 drop-shadow-sm">
                {selectedPolygon.name || "+ Add name"}
              </span>
              <div className="absolute inset-0 rounded-full bg-gradient-to-r from-white/10 to-white/5 group-hover:from-white/20 group-hover:to-white/10 transition-all duration-200"></div>
            </div>
          )}
      ...
      {buildSession.isActive && (
        <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex gap-2">
          <Button onClick={onCompleteBuildSession} size="sm" className="bg-green-600 hover:bg-green-700">
            Complete
          </Button>
          <Button onClick={onCancelBuildSession} size="sm" variant="outline">
            Cancel
          </Button>
        </div>
      )}
      ...
      {polygonSession.isActive && (
        <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex gap-2">
          <Button onClick={onCompletePolygonSession} size="sm" className="bg-green-600 hover:bg-green-700">
            Complete Polygon
          </Button>
          <Button onClick={onCancelPolygonSession} size="sm" variant="outline">
            Cancel
          </Button>
        </div>
      )}
      ...
      <div className="absolute bottom-4 right-4 flex flex-col gap-2">
        <Button onClick={onZoomIn} size="sm" variant="outline" className="p-2">
          <ZoomIn className="h-4 w-4" />
        </Button>
        <Button onClick={onZoomOut} size="sm" variant="outline" className="p-2">
          <ZoomOut className="h-4 w-4" />
        </Button>
        <Button onClick={onResetZoom} size="sm" variant="outline" className="text-xs px-2 py-1">
          Reset
        </Button>
      </div>
    </div>
